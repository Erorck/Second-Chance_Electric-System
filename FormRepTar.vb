...
#Region "Metodos para exportar datos"
    ...
    Public Sub exportardatospdf(ByVal document As Document, ByVal Header As String, ByVal Frase As String)
        ...
        Dim datatable As New PdfPTable(RepTarDataViewGrid.ColumnCount)
        datatable.DefaultCell.Padding = 3
        Dim headerwidths As Single() = Getcolumnaszise(RepTarDataViewGrid)
        ...
        For i As Integer = 0 To RepTarDataViewGrid.ColumnCount - 1
            datatable.AddCell(RepTarDataViewGrid.Columns(i).HeaderText)
        Next

        datatable.HeaderRows = 1
        datatable.DefaultCell.BorderWidth = 1

        For i As Integer = 0 To RepTarDataViewGrid.RowCount - 1
            For j As Integer = 0 To RepTarDataViewGrid.ColumnCount - 1
                datatable.AddCell(RepTarDataViewGrid(j, i).Value.ToString())
            Next
...
#Region "Generar PDF"
    Private Sub btnGenerarPDF_Click(sender As Object, e As EventArgs) Handles btnGenerarPDF.Click
        Try
            ...
            Dim filename As String = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\Reportes Tarifas.pdf"
            Dim file As New FileStream(filename, FileMode.Create, FileAccess.Write, FileShare.ReadWrite)
            PdfWriter.GetInstance(doc, file)
            doc.Open()
            exportardatospdf(doc, "Reporte", "Reporte de Tarifas")
            doc.Close()
